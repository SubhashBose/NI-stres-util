...
var (
	...
	FlagCPUglobalmaxpercent    = flag.Float64("cpu-m", 100.0, "Maximum limit of system's total CPU load percent to maintain. If other system processes consume CPU then the program will reduce CPU utilization in attempt to maintain this global limit.")
	FlagMemory                 = flag.Float64("mem", 0, "GiB of memory to use")
	FlagMemoryRefresh          = flag.Bool("mem-r", false, "Enabling this refreshes the memory every hour in attempt to prevent the OS from swapping out memory to disk and keep utilizing RAM.")
